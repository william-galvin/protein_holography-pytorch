...
def get_zernikegrams(nbs: np.array, # of custom dtype
    ...
    from e3nn import o3
    from protein_holography_pytorch.so3.functional import filter_by_l

    OnRadialFunctions = ZernickeRadialFunctions(rcut, rmax+1, 8, complex_sph = False)
    rst = RadialSphericalTensor(rmax+1, OnRadialFunctions, 8, 1, 1)
    ...
    zernikegrams = filter_by_l(torch.tensor(zernikegrams), data_irreps, lmax).numpy()
