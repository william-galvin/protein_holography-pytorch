...
class CGNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        for _ in range(self.n_cg_blocks):
            ...
            cg_blocks.append(CGNetBlock(prev_irreps,
                                                irreps_hidden,
                                                w3j_matrices,
        ...
        for _ in range(self.n_fc_blocks):
            block = []
            block.append(nn.Linear(prev_dim, self.h_dim))
            block.append(eval(NONLIN_TO_ACTIVATION_MODULES[self.fc_nonlin]))
        ...
        # output head
        self.output_head = nn.Linear(prev_dim, self.output_dim)
...
class SO3_ConvNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if self.do_initial_linear_projection:
            ...
            self.initial_linear_projection = linearity(self.irreps_in, initial_irreps)
